...
def evaluate(model_name):
    ...
    for dataset_name in dataset_list:
        ...
        # Use load test set to ensure balanced set
        # Pre-sample train_ordering datasets for each size
        ...
        for multiple in multiples:
            ...
            for set_run in tqdm(range(num_sets), desc=f"Processing sets for {dataset_name}"):
                ...
                orderings = generate_random_orderings([0] * num_incontext, num_orderings=num_runs_per_set)

                # Collectors for this set
                test_preds = []
                ...
                for perm_run in range(num_runs_per_set):
                    permuted_in_context = reorder_list(in_context_data, orderings[perm_run])
                ...
                # Compute majority vote accuracy for test set
                mv_correct = 0
                ...
                for size in train_ordering_train_sizes:
                    ...
                    best_alignment = -1
                    best_train_idx = 0
                    for i in range(num_runs_per_set):
                ...
                # Collect example_to_order accuracies
                example_to_order_accuracies.extend(test_accs)
            
        # Save predictions after processing all permutations for this set
        prediction_df = pd.DataFrame(all_predictions)
        prediction_df.to_csv(
            f'{output_base}/generated/{dataset_name}_all_predictions.csv',
            index=False
        )

        # Save training predictions split by size
        train_pred_df = pd.DataFrame(all_train_predictions)
        for size in train_ordering_train_sizes:
            # Filter predictions for this size
            size_df = train_pred_df[train_pred_df['training_set_size'] == size]
            size_filename = f'{dataset_name}_{size}_train_predictions.csv'
            size_df.to_csv(
        ...
        for exp_name, accs in results.items():
            if exp_name == 'train_ordering':
                for size, size_accs in accs.items():
                    ...
                    size_filename = f'{dataset_name}_{size}_results.csv'
                    df.to_csv(f'{output_base}/results/{exp_name}/{size_filename}', index=False)
                    
                    plt.figure(figsize=(10,6))
                    plt.boxplot(size_accs)
                    plt.title(f'Train Ordering Accuracy (Size {size}) - {dataset_name}')
                    plt.ylabel('Accuracy')
                    plt.savefig(f'{output_base}/boxplots/{dataset_name}_train_ordering_{size}_boxplot.png')
                    plt.close()

                    plt.figure(figsize=(10,6))
                    plt.hist(size_accs, bins=20, edgecolor='black')
                    plt.title(f'Train Ordering Accuracy Frequency (Size {size}) - {dataset_name}')
                    plt.xlabel('Accuracy')
                    plt.ylabel('Frequency')
                    plt.savefig(f'{output_base}/frequency_graphs/{dataset_name}_train_ordering_{size}_histogram.png')
                    plt.close()
            else:
                # Save CSV
                df = pd.DataFrame({'Accuracy': accs})
                df.to_csv(f'{output_base}/results/{exp_name}/{dataset_name}_results.csv', index=False)
                
                # Generate plots
                plt.figure(figsize=(10,6))
                plt.boxplot(accs)
                plt.title(f'{exp_name} Accuracy Distribution - {dataset_name}')
                plt.ylabel('Accuracy')
                plt.savefig(f'{output_base}/boxplots/{dataset_name}_{exp_name}_boxplot.png')
                plt.close()

                plt.figure(figsize=(10,6))
                plt.hist(accs, bins=20, edgecolor='black')
                plt.title(f'{exp_name} Accuracy Frequency - {dataset_name}')
                plt.xlabel('Accuracy')
                plt.ylabel('Frequency')
                plt.savefig(f'{output_base}/frequency_graphs/{dataset_name}_{exp_name}_histogram.png')
                plt.close()
